    ...
    public function getList(){
        ...
        foreach($infos as $info){
            ...
            $out_item['properties'] = array(
                ...
                'risque_integration_bati'=>!is_null($info->getRisqueIntegrationBati())? $info->getRisqueIntegrationBati()->getLibelle(): 'null',
                'risque_deplacement'=>!is_null($info->getRisqueDeplacement())? $info->getRisqueDeplacement()->getLibelle(): 'null',
                'risque_integration_topo'=>!is_null($info->getRisqueIntegrationTopo())? $info->getRisqueIntegrationTopo()->getLibelle(): 'null',
                'risque_integration_vegetation'=>!is_null($info->getRisqueIntegrationVegetation())? $info->getRisqueIntegrationVegetation()->getLibelle(): 'null',
                'zone_sensible'=>!is_null($info->getZoneSensible())? $info->getZoneSensible()->getLibelle():'null',
            );
            $out_item['geometry'] = $info->getGeomJson();
            $out_item['properties']['geomLabel'] = sprintf('<a href="#/cables/tronconserdf/%s">%s</a>',
